...
import { MenteeFormValues, MentorFormValues } from "@/schemas/signup";

// The mutation now expects an object containing the type and payload
...
export const useSignUp = (
  ...
  return useMutation<void, Error, ApplicationPayload>({
    mutationFn: async ({ payload, type }) => {
      // 1. Choose the correct Firestore collection based on the user type
      const collectionName =
        type === "mentor" ? "mentorApplications" : "menteeApplications";
      const applicationCollection = collection(firebaseDB, collectionName);
      ...
      // 2. Save the application to the correct collection
      await addDoc(applicationCollection, firestoreData);

      // 3. Queue dynamic emails for the applicant and admin
      const mailCollection = collection(firebaseDB, "mail");

      const applicantEmailData = {
        to: payload.email,
        message: {
          subject: `Application Received - FrankMolla ${
            type === "mentor" ? "Mentor" : "Mentorship"
          }`,
          html: `<p>Hi ${payload.fullName},</p><p>Thank you for submitting your application to become a ${type}. We will review it and be in touch soon.</p>`,
        },
      };
      ...
      const adminEmailData = {
        to: ADMIN_EMAIL,
        message: {
          subject: `NEW ${type.toUpperCase()} Application: ${payload.fullName}`,
          html: `<p>A new <strong>${type}</strong> application has been submitted by ${payload.fullName} (${payload.email}). Please review it in the '${collectionName}' collection.</p>`,
        },
      };
